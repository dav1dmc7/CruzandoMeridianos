...
const fetchSource = async (source) => {
  ...
  if (text.length < 120) {
    throw new Error(`${source.id}: source returned insufficient text`);
  }
...
const generateAlerts = async (destination, sources) => {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error("OPENAI_API_KEY is required to generate traveler alerts");
  }
...
const loadPrevious = async () => {
  try {
    const moduleUrl = `${pathToFileURL(generatedPath).href}?cacheBust=${Date.now()}`;
...
for (const destination of TRAVEL_INTELLIGENCE_SOURCES) {
  ...
  if (changed.length > 0) {
    ...
    const newAlerts = (aiResult.alerts ?? [])
      ...
      .map((alert) => {
        const source = fetched.find((candidate) => candidate.id === alert.sourceId);
        return normalizeAlert(alert, source);
      });
